...
このとき、$f$ の型は $\forall \beta . \beta \to (\alpha \times \beta)$
であるはずだという予想ができます。$\beta$ は任意の型となっていますが、$\alpha$ はなっていません。

これは、$\alpha$ が let の外側の型環境に含まれていることによる制限です。
外にもある型変数を勝手に付け替えてしまえば、型の整合性が取れなくなってしまいます。

よって、
$\forall$ の後に書かれる型変数は多相変数の束縛の外側の型環境に含まれていてはならないことが分かります。
この他に制約はありませんが、型に含まれない型変数を任意の型としてしまっても意味が無いので、
型の中に含まれていてかつ外側の型環境に含まれない型変数を任意の型として良いと考えるのが良いでしょう。
