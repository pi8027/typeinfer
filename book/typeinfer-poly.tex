...
しかし、第2章で示したような制約の生成と制約解消を完全に分離した形の推論アルゴリズムでは、
...
\texttt{Int.Set.t}は整数(\texttt{int}型の値)の集合を表す型です。
...
今までの幾つかの定義とは違い、型環境が持つのは型スキームです。
...
\texttt{Int.Set.singleton}関数は\texttt{int}型の値を取り、その値のみを要素とする集合を返す関数です。
\texttt{Int.Set.union}関数は二つの\texttt{int}型の値の集合を取り、それらの和集合を返す関数です。
...
文字列からのマップの全要素に対するマップ関数\texttt{String.Map.map}を用いてこのように書けます。

代入を合成する関数\texttt{merge\_substs}をリスト\ref{list:ocaml-merge-subst}のように定義します。

\begin{lstlisting}[caption=代入の合成, label=list:ocaml-merge-subst]
