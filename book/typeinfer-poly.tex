...
let による最初の束縛は真に一致するものであると言えますが、
...
項が let 項 $\letterm{x}{e_1}{e_2}$ の場合も、
...
アルゴリズム $\mathcal W$ を OCaml で実装してみましょう。
...
\begin{lstlisting}[caption=型に含まれる変数集合の計算, label=list:ocaml-freevars]
...
\begin{lstlisting}[caption=代入リストの合成, label=list:ocaml-merge-substs]
