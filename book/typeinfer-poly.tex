...
最も簡単な let 項に対する型推論は、let で束縛された項に関する等式集合をその場で生成せずに、
その変数が使われる度に等式集合を生成することで実現できます。

$\lambda$ 抽象で束縛される変数は今まで通り型環境にその変数に割り当てられた型を持つことになりますが、
let 項で束縛される変数に関してはは制約の生成に必要な情報を持つことになります。
制約の生成に必要なのは型環境と項なので、それらのペアを型環境に持っておくことにしましょう。
