...
パラメトリック多相を持つ型システムに対する型推論で最も重要なのは、多相型を作る計算、
...
制約の生成の途中で多相にして良い型変数を判定できず、多相型を作る計算ができません。
この問題を解決するためには、多相型を作る時点でその部分の制約を解消する必要があります。
  ...
  \begin{align*}
    ...
    \begin{cases}
      ...
      \tau [\alpha := \mathrm{fresh}, \dots, \beta := \mathrm{fresh}]
        & (\forall \alpha \dots \beta . \tau = \Gamma(x))
    \end{cases} \\
    \mathcal C^\mathrm{let'}(\Gamma, e_1 \, e_2) &=
    \begin{array}[t]{l}
      \mathrm{let}
    ...
    \begin{array}[t]{l}
      \mathrm{let}
        \begin{array}[t]{l}
          ...
          \sigma_1      = \forall (FTV(\tau_1') - FTV(\Gamma)) . \tau_1' \\
          (c_2, \tau_2) = \mathcal C^\mathrm{let'}((\Gamma, x : \sigma_1) , e_2)
        \end{array} \\
...
このアルゴリズムでは、項が変数であるときに多相型の任意の型にフレッシュな型を代入しています。
また、項が let 項であるときに多相になる項の制約生成で得られた制約をアルゴリズム $\mathcal U$
によって解き、その結果を用いて多相型を作っています。
