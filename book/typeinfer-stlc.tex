...
まず、二番目の書き換え規則で使われている型の代入の計算を実装しましょう。
この関数の名前を \texttt{substitute} とします。

広い範囲に適用できるようにするため、代入(\texttt{subst} 型の値)を取って、
それに対応する型から型への関数を作る形にしましょう。
代入と型を取って、型に代入を適用した型を返すと言っても同じことになります。
以上のことから、\texttt{substitute} 関数は \texttt{subst -> ty -> ty} 型を持ちます。

この方針に基き代入を実装すると、リスト\ref{list:ocaml-stlc-substitute}のようになります。
